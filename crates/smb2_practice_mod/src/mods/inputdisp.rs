    ...
    fn get_gradient(&self, cx: &Context) -> Gradient {
        match InputDispColorType::from_pref(U8Pref::InputDispColorType, cx.pref) {
            InputDispColorType::Default => {
                Self::COLOR_MAP[cx.pref.get(U8Pref::InputDispColor) as usize].into()
            }
            ...
            InputDispColorType::RgbGradient => {
                ...
                let color2 = mkb::GXColor {
                    r: cx.pref.get(U8Pref::InputDispGradientColor2Red),
                    g: cx.pref.get(U8Pref::InputDispGradientColor2Green),
                    b: cx.pref.get(U8Pref::InputDispGradientColor2Blue),
                    a: 0xff,
                };
                let rotation = cx.pref.get(U8Pref::InputDispGradientRotation);
                let rotation = math::map_range(
                    rotation as i32,
                    0..100,
                    (i16::MIN as i32)..(i16::MAX as i32),
                ) as i16;
                let start = cx.pref.get(U8Pref::InputDispGradientStart) as f32 / 100.0;
                let end = cx.pref.get(U8Pref::InputDispGradientEnd) as f32 / 100.0;
                Gradient {
                    color1,
                    color2,
                    rotation,
