use crate::{cstr, fmt, fmt_buf, systems::draw};
...
pub fn format_timer(frames: i32, format: Format, out_buf: &mut ArrayString<32>) {
    ...
    } else if format == Format::Full && minutes > 0 {
        &fmt_buf!(
            out_buf,
            c"%s%02d:%02d.%02d",
            cstr!(8, sign),
