...
static bool appliedBoost = false;
...
static void jumping() {
    ...
    if (s_jumping && !paused_now) {
        ...
        lerp = lerp * lerp * lerp;
        ball.vel.y += lerp * 0.1 * 0.5;
        if(!appliedBoost){
            Vec absVel = {};
            absVel.z = -mkb::math_sqrt(ball.vel.x * ball.vel.x + ball.vel.z * ball.vel.z); // Get the absolute val of x-z plane velocity AS A VEC POINTING -Z
            s32 stickX = mkb::pad_status_groups[0].raw.stickX; // Make stickX easier to call
            s32 stickY = mkb::pad_status_groups[0].raw.stickY; // Make stickY easier to call
            s32 absStick = mkb::math_sqrt(stickX * stickX + stickY * stickY); // Get the absolute val of stick strength
            s32 stickRot = mkb::math_atan2(-stickX, stickY); // Get the rotation of the stick
            absVel.z -= absStick * 0.001; // Amplify vec by stick strength
            mkb::mtxa_from_rotate_y(mkb::g_current_camera->rot.y); // MtxA rotates by cam Y
            mkb::mtxa_rotate_y(stickRot); // MtxA also rotates by stick rot
            mkb::mtxa_tf_vec(&absVel, &absVel); // Apply the matrix to the vec
            ball.vel.x = absVel.x;
            ball.vel.z = absVel.z;
            appliedBoost = true;
        }
