...
static void handle_widget_bind() {
    ...
    switch (selected->type) {
        ...
        case WidgetType_FloatEdit: {
            ...
            if (Pad_ButtonReleased(mkb_PAD_BUTTON_A, false) && s_edit_tick > 0) {
                s_edit_tick = 0;
            } else if (Pad_ButtonReleased(mkb_PAD_BUTTON_Y, false) && s_edit_tick < 0) {
                s_edit_tick = 0;
            }
        ...
        case WidgetType_InputSelect: {
            ...
            if (s_binding == BindingState_Requested &&
                ...
                // set new bind
                Binds_Encoding type = Binds_GetEncodingType();
...
void MenuImpl_Tick() {
    ...
    if (!s_visible) {
        ...
        if (Pad_ButtonChordPressed(mkb_PAD_TRIGGER_L, mkb_PAD_TRIGGER_R, true) &&
            ...
            Draw_NotifyWithDuration(COLOR_RED, Draw_NotifyDuration_Long, "Use %s to toggle menu",
                                    buf);
        }
        return;
    }
...
static void draw_help(const Widget *widget) {
    ...
    switch (widget->type) {
        ...
        case WidgetType_Choose: {
            ...
            Draw_DebugText(BUTTON_START + 3 * BLOCK_WIDTH + HALF_SPACE, Y_HEIGHT, COLOR_WHITE,
                           "Reset");
            break;
        }
        case WidgetType_IntEdit:
...
static void draw_widget(Widget *widget, u32 selected_idx, u32 *selectable_idx, u32 *y,
    ...
    switch (widget->type) {
        ...
        case WidgetType_ColoredText: {
            const ColoredTextWidget *colored_text = &widget->colored_text;
            if (colored_text->overlap) *y -= LINE_HEIGHT;

            u32 len = mkb_strlen((char *)colored_text->label);
            for (u32 i = 0; i < len; i++) {
                f32 t = len > 1 ? (f32)i / (f32)(len - 1) : 0.f;
                GXColor color =
                    Draw_LerpColors(t, colored_text->color_left, colored_text->color_right);
                char ch[2] = {colored_text->label[i], '\0'};
                Draw_DebugText(MARGIN + PAD + (colored_text->offset_x + i) * DRAW_DEBUG_CHAR_WIDTH,
                               *y, color, "%s", ch);
            }
        ...
        case WidgetType_IntEdit: {
            ...
            char value_buf[32];
            if (widget->int_edit.format != nullptr) {
                widget->int_edit.format(Pref_Get(widget->int_edit.pref), value_buf);
            } else {
        ...
        case WidgetType_RgbPreview: {
            GXColor color = {
                .r = (u8)Pref_Get(widget->rgb_preview.r_pref),
                .g = (u8)Pref_Get(widget->rgb_preview.g_pref),
                .b = (u8)Pref_Get(widget->rgb_preview.b_pref),
                .a = 0xff,
            };
            f32 x1 = 400.f;
            f32 y1 = (f32)(*y) + 1.f;
...
static u32 draw_breadcrumbs_from(u32 start, void (*draw_fn)(u32, u32, GXColor, const char *)) {
    ...
    if (start > 0) {
        for (u32 i = 0; i < 3; i++) {
            Draw_DebugText(x, MARGIN + PAD, grey, ".");
            x += 6;
        }

        draw_fn(x, MARGIN + PAD, COLOR_BLUE, ARROW_STR);
        x += mkb_strlen((char *)ARROW_STR) * DRAW_DEBUG_CHAR_WIDTH;
    }

    for (u32 i = start; i <= s_menu_stack_ptr; i++) {
        MenuWidget *menu = s_menu_stack[i];
        draw_fn(x, MARGIN + PAD, i == s_menu_stack_ptr ? COLOR_PURPLE : grey, menu->label);
...
static void draw_breadcrumb_noop(u32 x, u32 y, GXColor color, const char *text) {
    (void)x;
    (void)y;
    (void)color;
    (void)text;
...
static void draw_breadcrumb_text(u32 x, u32 y, GXColor color, const char *text) {
    Draw_DebugText(x, y, color, "%s", text);
}

static void draw_breadcrumbs() {
    for (u32 i = 0; i <= s_menu_stack_ptr; i++) {
        // Calculate breadcrumb width to see if it would overflow
        u32 final_x = draw_breadcrumbs_from(i, draw_breadcrumb_noop);
        if (final_x <= SCREEN_WIDTH - MARGIN - PAD) {
            draw_breadcrumbs_from(i, draw_breadcrumb_text);
            break;
        }
    }
