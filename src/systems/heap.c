...
mkb_ChunkInfo *heap_extract_chunk(mkb_ChunkInfo *list, mkb_ChunkInfo *chunk) {
    if (chunk->next) {
...
mkb_ChunkInfo *heap_add_chunk_to_front(mkb_ChunkInfo *list, mkb_ChunkInfo *chunk) {
    chunk->next = list;
    chunk->prev = nullptr;
...
mkb_ChunkInfo *heap_find_chunk_in_list(mkb_ChunkInfo *list, mkb_ChunkInfo *chunk) {
...
static void make_heap() {
    u32 start = mkb_OSRoundUp32B(*(u32 *)Rel_RelocateAddr(0x8000452C));
    void *end_ptr = Rel_ComputeMainLoopRelDataBoundary((void *)start);
...
void *Heap_Alloc(u32 size) {
    ...
    if (leftover_size < min_size) {
        ...
        s_heap_info->first_free = heap_extract_chunk(s_heap_info->first_free, temp_chunk);
    } else {
...
bool Heap_Free(void *ptr) {
    ...
    if (!heap_find_chunk_in_list(s_heap_info->first_used, temp_chunk)) {
        return false;
    }

    // Extract the chunk from the allocated list
    s_heap_info->first_used = heap_extract_chunk(s_heap_info->first_used, temp_chunk);
...
static bool check_if_pointer_is_valid(void *ptr) {
    u32 ptr_raw = (u32)ptr;
...
void Heap_Init() {
    ...
    s_heap_info = ModLink_GetHeapInfo();
