...
unsafe fn extract_chunk(
    list: *mut mkb2::ChunkInfo,
    chunk: *mut mkb2::ChunkInfo,
) -> *mut mkb2::ChunkInfo {
...
pub struct Heap {
    owned_heap_info: UnsafeCell<mkb2::HeapInfo>,
    heap_info: UnsafeCell<*mut mkb2::HeapInfo>,
}

unsafe fn make_heap_info() -> mkb2::HeapInfo {
    ...
    let heap_info = mkb2::HeapInfo {
        capacity: size as u32,
        first_free: start as *mut mkb2::ChunkInfo,
        first_used: null_mut(),
    };
...
unsafe impl GlobalAlloc for Heap {
    unsafe fn alloc(&self, layout: core::alloc::Layout) -> *mut u8 {
        ...
        let new_size = new_size + math::round_up_pow2(core::mem::size_of::<mkb2::ChunkInfo>(), 32);
        ...
        let min_size = math::round_up_pow2(core::mem::size_of::<mkb2::ChunkInfo>(), 32) + 32;
        ...
        if leftover_size < min_size as isize {
            ...
            // Create a new chunk
            let new_chunk = (temp_chunk as usize + new_size) as *mut mkb2::ChunkInfo;
        ...
        let allocated_memory = (temp_chunk as usize
            + math::round_up_pow2(core::mem::size_of::<mkb2::ChunkInfo>(), 32))
            as *mut u8;
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, _layout: core::alloc::Layout) {
        ...
        let header_size = math::round_up_pow2(core::mem::size_of::<mkb2::ChunkInfo>(), 32);

        // Remove the header size from ptr, as the value stored in the list does not include it
        let temp_chunk = (ptr_raw - header_size) as *mut mkb2::ChunkInfo;
        ...
        heap_info.first_free = mkb2::DLInsert(heap_info.first_free, temp_chunk);
    }
}

#[global_allocator]
pub static HEAP: Heap = Heap {
    owned_heap_info: UnsafeCell::new(mkb2::HeapInfo {
        capacity: 0,
        first_free: null_mut(),
        first_used: null_mut(),
    }),
