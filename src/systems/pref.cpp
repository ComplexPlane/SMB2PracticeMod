...
static const DefaultPref DEFAULT_PREFS[] = {
    ...
    {Pref::FreecamSpeedMult, 3},         // 3
    {Pref::MenuBind, 64},                // L + R
    {Pref::Friction, 110},               // 0.10
    {Pref::Restitution, 150},            // 0.5
    {Pref::IlBattleReadyBind, 104},      // dpad-down
    {Pref::FreecamToggleBind, 255},      // unbound
    {Pref::SavestateClearBind, 255},     // unbound
    {Pref::SavestateClearAllBind, 255},  // unbound
...
static u8 s_pref_state[MAX_PREFS];
...
static void card_buf_to_pref_struct(void *card_buf) {
    ...
    for (u32 i = 0; i < header->num_prefs; i++) {
        ...
        if (static_cast<u16>(id) < LEN(s_pref_state)) {
            s_pref_state[static_cast<u16>(id)] = static_cast<u8>(pref_data);
        }
    }
}
...
static void pref_struct_to_card_buf() {
    ...
    for (u32 i = 0; i < LEN(PREF_IDS); i++) {
        ...
        entry_list[i].data = s_pref_state[static_cast<u16>(PREF_IDS[i])];
    }
}
