...
void TimerDisp_DrawTimer(s32 frames,
    ...
    if (hours > 0 && !show_seconds) {
        ...
        Draw_DebugText(X + 48, y, color, "%s%d:%02d:%02d.%02d", sign, hours, minutes, seconds,
                       centiseconds);
    } else if (minutes > 0 && !show_seconds) {
        Draw_DebugText(X, y, color, prefix);
        Draw_DebugText(X + 48, y, color, "%s%02d:%02d.%02d", sign, minutes, seconds, centiseconds);
    } else {
        u32 total_seconds =
            seconds + (minutes * MINUTE_FRAMES + hours * HOUR_FRAMES) / SECOND_FRAMES;
        Draw_DebugText(X, y, color, prefix);
        Draw_DebugText(X + 48, y, color, "%s%02d.%02d", sign, total_seconds, centiseconds);
    }
...
void TimerDisp_DrawPercentage(s32 fsave, const char *prefix, u32 row, GXColor color) {
    ...
    mkb_set_ui_widescreen_scale_mtx(0);
}
