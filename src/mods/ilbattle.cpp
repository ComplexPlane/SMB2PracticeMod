...
static u32 convert_battle_length(u8 battle_length_choice) {
    if (battle_length_choice == 0)
        return 5 * 60 * 60;
    else if (battle_length_choice == 1)
        return 7 * 60 * 60;
    else if (battle_length_choice == 2)
        return 10 * 60 * 60;
    else
        return 0;
}
...
static void run_battle_timer() {
    if (pref::get_il_battle_length() != 3) {  // If the timer isnt endless
        ...
        battle_frames++;  // If the timer is endless
}
...
static void track_best() {
    ...
    if (mkb::sub_mode == mkb::SMD_GAME_GOAL_INIT ||
        ...
        if (current_frames > best_frames && !invalid_pause) {  // New best time test, & invalid
                                                               // pause test
            best_frames = current_frames;
        }
        if (score_calc(current_score) > best_score && !invalid_pause) {  // New best score test, &
                                                                         // invalid pause test
...
void tick() {
    if (pref::get_il_battle_display()) {
        if (s_state == IlBattleState::WaitForFirstRetry) {
            ...
            track_best();
            run_battle_timer();  // To-Do: Implement realtime timer (without loads), if I figure out
                                 // how
        } else if (s_state == IlBattleState::BuzzerBeater) {
            track_invalid_pauses();
            track_final_attempt();
        } else if (s_state == IlBattleState::BattleDone) {  // DELETE THIS IF THERE'S NOTHING HERE
                                                            // LATER
        }
